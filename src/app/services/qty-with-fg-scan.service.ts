...
export class QtyWithFGScanService {
  ...
  httpOptions = {
    ...
    //Save Data
    public saveBatchSerInfo(CompanyDBID:string,FGBatchSer:string,QtyProd:number,IsRejected:boolean,IsNC:boolean,WorkOrderNo:string,ItemCode:string,OperationNo:string,loggedInUser:string):Observable<any>{
    //JSON Obeject Prepared to be send as a param to API
    let jObject:any={ MoveOrder: JSON.stringify([{ CompanyDBID: CompanyDBID,WorkOrder: WorkOrderNo,FGBatchSerial:FGBatchSer,Rejected: IsRejected,User: loggedInUser,NC:IsNC,Item:ItemCode,Operation:OperationNo,Quantity:QtyProd }]) };
    //Return the response form the API  
